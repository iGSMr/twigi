...
def main(args,AUTH=None, update_outputtarget=False):
    target_id = None
    server_url = None
    try:
        tw_consumer_key = args["twitter"]["CONSUMER_KEY"]
        ...
        sys.exit(1)
    #if "auth" in config:
    #    AUTH.append(storyconfig_tools.login(config))
    ...
    try:
        ...
        if AUTH is not None:
            logger.info("Setting authenticator")
            s.httpclient.authenticator = AUTH.get_auth_header
    #    for a in AUTH:
    #        s.httpclient.authenticator = a.get_auth_header
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument("-l", "--loop", dest="loop", action="store_true",
                            help="If set, loop until killed, or stopped by pressing Ctrl-C")
        parser.add_argument("--config_encoding", dest="config_encoding", action="store", default="UTF-8",
                            help="Override the default UTF-8 encoding for reading configuration files",
                            metavar="ENCODING")

        args = parser.parse_args()
        config = storyconfig_tools.read_config(args.configfile, args.config_encoding)
        loop = args.loop
        if loop:
            logger.info("The agent will loop until it's killed or stopped by pressing Ctrl+C")
        retry = True
        if "auth" in config:
            authenticator = storyconfig_tools.login(config)
        while retry:
            retry = loop
            config = ConfigParser()
            config.read(args.configfile)
            main(config, authenticator)
            if loop:
                logger.info("Looping in 5 seconds. Press Ctrl+C to stop.")
                time.sleep(5)
